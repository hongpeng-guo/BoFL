...
def plot_energy_curve_by_round(device, task_name, max_ddl):
    ...
    x = np.arange(1, 41)
    ax1.plot(x, np.array(baseline), marker=markers[0], markersize=6,  label=r'\texttt{Performant}',linestyle="--", color=colors[0])
    ax1.plot(x, np.array(oracle), marker=markers[2], markersize=6,  label=r'\texttt{Oracle}', linestyle="-", color=colors[1])
    ax1.plot(x, np.array(bofl), marker=markers[1], markersize=6,  label=r'\texttt{BoFL}', linestyle=":", color=colors[2])
    ...
    ax1.set_ylabel('Energy Consumed (J)', fontsize='20')
    ...
    ax2.set_xlabel('Round Number', fontsize='20')
    ax2.set_ylim(Data[d][t][m]['min_ddl'], Data[d][t][m]['min_ddl'] * max_ddl)
    ax2.set_xlim(0, 41)
    ax2.set_ylabel('DDL (s)', fontsize='20')
    ax2.yaxis.set_major_formatter(mpl.ticker.StrMethodFormatter('{x:4.0f}'))
    # ax2.grid(axis='y', linestyle='--', linewidth=1)

    legend = fig.legend(ncol=3, fancybox=True, fontsize='20', loc='lower center',  bbox_to_anchor=(0., 0.845, 1.0, 1.0))
...
def plot_pareto_graph(device, task_name, max_ddl):
    ...
    ax.scatter(all_points[:, 0], all_points[:, 1], marker='.',  s=300, lw=0, color="grey", alpha=0.4, label='Unexplored Configurations')
    ax.scatter(tried_points[:, 0], tried_points[:, 1], marker='o', s=300, lw=0,  alpha=0.5, label='BoFL Explorations')
    ax.scatter(tried_pareto[:, 0], tried_pareto[:, 1], marker='s', s=300, lw=0, color="blue", alpha=0.8, label='BoFL Pareto Front')
    ax.scatter(real_pareto[:, 0], real_pareto[:, 1], marker='*', s=300, lw=0,  color="red", alpha=0.8, label='Actual Pareto Front')
    ax.set_ylabel('Energy Consumption per minibatch (J)', fontsize='22')
    ax.set_xlabel('Execution Latency per minibatch (s)', fontsize='22')
    ...
    ax.text(1.03 * xmin, 1.03 *ymin, "Better", ha="left", va="bottom", rotation=45, size=24,
            bbox=dict(boxstyle="larrow,pad=0.3", fc="cyan", ec="black", lw=2))

    legend = fig.legend(ncol=1, fancybox=True, fontsize='22', loc='lower left',  bbox_to_anchor=(0.1, 0.65, 1.0, 1.0))
    legend.get_frame().set_edgecolor('black')
    plt.tight_layout()
...
def plot_energy_w_ddl_max():
    ...
    Plot_data = [[[None for _ in range(5)] for _ in range(2)] for _ in range(3)]
    ...
    fig, axs = plt.subplots(3,2,figsize=(12,6), gridspec_kw={'height_ratios': [1, 1, 1], 'hspace': 0.5}, sharex=True)
    
    width = 0.4

    x = np.arange(5)
    axs[0][1].set_title(r'\textbf{(b) CIFAR10-ViT}', fontsize='18')
    axs[0][1].set_xticks(x)
    axs[0][1].bar(x , 100 * Plot_data[0][0], width, edgecolor='black', color=colors[0])
    axs[0][1].yaxis.tick_right()
    axs[0][1].set_xticklabels(['{:1.1f}x'.format(ddl) for ddl in Ddls])
    axs[0][1].yaxis.set_major_formatter(mpl.ticker.StrMethodFormatter('{x:1.0f}'))
    axs[0][1].yaxis.grid(linestyle='--',  linewidth=1)

    axs[1][1].set_title(r'\textbf{(d) ImageNet-ResNet50}', fontsize='18')
    axs[1][1].set_xticks(x)
    axs[1][1].bar(x , 100 * Plot_data[0][0], width, edgecolor='black', color=colors[0])
    axs[1][1].set_xticklabels(['{:1.1f}x'.format(ddl) for ddl in Ddls])
    axs[1][1].yaxis.tick_right()
    axs[1][1].set_ylabel(r'Regret Compared to \texttt{Oracle} (\%)', fontsize='22')
    axs[1][1].yaxis.set_major_formatter(mpl.ticker.StrMethodFormatter('{x:1.0f}'))
    axs[1][1].yaxis.grid(linestyle='--',  linewidth=1)

    axs[2][1].set_title(r'\textbf{(f) IMDB-LSTM}', fontsize='18')
    axs[2][1].set_xticklabels(['{:1.1f}x'.format(ddl) for ddl in Ddls])
    axs[2][1].yaxis.tick_right()
    axs[2][1].bar(x , 100 * Plot_data[0][0], width, edgecolor='black', color=colors[0])
    axs[2][1].yaxis.set_major_formatter(mpl.ticker.StrMethodFormatter('{x:1.0f}'))
    axs[2][1].yaxis.grid(linestyle='--',  linewidth=1)


    axs[0][0].set_title(r'\textbf{(a) CIFAR10-ViT}', fontsize='18')
    axs[0][0].set_xticklabels(['{:1.1f}x'.format(ddl) for ddl in Ddls])
    axs[0][0].bar(x , 100 * Plot_data[0][1], width, edgecolor='black', color=colors[1], hatch='/')
    axs[0][0].set_xticklabels(Ddls)
    axs[0][0].yaxis.set_major_formatter(mpl.ticker.StrMethodFormatter('{x:2.0f}'))
    axs[0][0].yaxis.grid(linestyle='--',  linewidth=1)

    axs[1][0].set_title(r'\textbf{(c) ImageNet-ResNet50}', fontsize='18')
    axs[1][0].set_xticklabels(['{:1.1f}x'.format(ddl) for ddl in Ddls])
    axs[1][0].bar(x , 100 * Plot_data[0][1], width, edgecolor='black', color=colors[1], hatch='/')
    axs[1][0].set_xticklabels(Ddls)
    axs[1][0].set_ylabel(r'Improvement Compared to \texttt{Performant} (\%)', fontsize='22')
    axs[1][0].yaxis.set_major_formatter(mpl.ticker.StrMethodFormatter('{x:2.0f}'))
    axs[1][0].yaxis.grid(linestyle='--',  linewidth=1)

    axs[2][0].set_title(r'\textbf{(e) IMDB-LSTM}', fontsize='18')
    axs[2][0].set_xticklabels(['{:1.1f}x'.format(ddl) for ddl in Ddls])
    axs[2][0].bar(x , 100 * Plot_data[0][1], width, edgecolor='black', color=colors[1], hatch='/')
    axs[2][0].yaxis.set_major_formatter(mpl.ticker.StrMethodFormatter('{x:2.0f}'))
    axs[2][0].yaxis.grid(linestyle='--',  linewidth=1)

    fig.text(0.5, 0.02, r'Normalized Maximun Deadline Length ($T_{max}/T_{min}$)', va='center', ha='center', fontsize='22')
...
def plot_Bayesain_overhead_per_round():
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(5,5),gridspec_kw={'hspace': 0.5})
    labels = ['TX2', 'AGX']
    x = np.arange(len(labels))
    ax1.set_xticks(x)
    ax1.set_xticklabels(labels)
    ax1.bar(0, TX2_latency_mean - 5, yerr=TX2_latency_error, width=0.3, hatch=None, edgecolor='black', color=colors[3])
    ax1.bar(1, AGX_latency_mean - 2, yerr=AGX_latency_error, width=0.3, hatch='/', edgecolor='black', color=colors[4])
...
def plot_Bayesain_overhead_percentail():
    ...
    legend = fig.legend(ncol=1, fancybox=True, fontsize='20')
    legend.get_frame().set_edgecolor('black')
    plt.tight_layout()
...
if __name__ == '__main__':
    ...
    plot_energy_curve_by_round('AGX', 'IMDB', 4.0)
    plot_energy_curve_by_round('AGX', 'CIFAR10', 4.0)
    plot_energy_curve_by_round('AGX', 'ImageNet', 4.0)
    plot_energy_curve_by_round('TX2', 'IMDB', 4.0)
    plot_energy_curve_by_round('TX2', 'CIFAR10', 4.0)
    plot_energy_curve_by_round('TX2', 'ImageNet', 4.0)

    plot_pareto_graph('AGX', 'IMDB', 4.0)
    plot_pareto_graph('AGX', 'CIFAR10', 4.0)
    plot_pareto_graph('AGX', 'ImageNet', 4.0)
    plot_pareto_graph('TX2', 'IMDB', 4.0)
    plot_pareto_graph('TX2', 'CIFAR10', 4.0)
    plot_pareto_graph('TX2', 'ImageNet', 4.0)
